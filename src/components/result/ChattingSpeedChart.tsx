        ...
        <div style={{marginTop: 40}}>
            ...
            <ResponsiveContainer height={320}>
                <LineChart data={chattingSpeed} margin={{ top: 20, left: -20, right: 20 }}>
                    ...
                    <Tooltip labelFormatter={(label: number | string) => {
                        const chat = chattingSpeed.find(e => e.time === label);
                        return typeof label === "number" ? 
                        <> {format(label, 'HH:mm:ss')} <br /> 
                            {chat?.allTexts.join(", ")} <br /> 
                            {chat?.laughTexts.join(", ")} <br /> 
                            {chat?.youtubeTexts.join(", ")} <br /> 
                            {chat?.surpriseTexts.join(", ")} <br /> 
                            {chat?.exclamationMarkTexts.join(", ")} <br /> 
                            {chat?.questionMarkTexts.join(", ")} <br /> 
                        </> : null
                        }}
                    />
                    {renderAllChart && <Line dataKey="allFrequency" dot={false} stroke="grey" fillOpacity={0.2} name="전체 채팅 수" />}
                    {renderLaughChart && <Line dataKey="laughFrequency" dot={false} stroke="blue" fillOpacity={0.2} name="ㅋㅋㅋㅋ" />}
                    {renderYoutubeChart && <Line dataKey="youtubeFrequency" dot={false} stroke="red" fillOpacity={0.2} name="유튜브 언급" />}
                    {renderSurpriseChart && <Line dataKey="surpriseFrequency" dot={false} stroke="orange" fillOpacity={0.2} name="감탄" />}
                    {renderExclamationMarkChart && <Line dataKey="exclamationMarkFrequency" dot={false} stroke="green" fillOpacity={0.2} name="느낌표" />}
                    {renderQuestionMarkChart && <Line dataKey="questionMarkFrequency" dot={false} stroke="purple" fillOpacity={0.2} name="물음표" />}
                </LineChart>
            </ResponsiveContainer>
        </div>
        ) : null;
