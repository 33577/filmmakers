...
export const getChattingSpeed = (
    chatLog: string,
    keyword: FilteringKeyword | FilteringKeyword[] = "ALL",
    timeIntervalInMs: number = 10000,
): ChattingSpeed => {
    ...
    return parsedChatList.reduce<ChattingSpeed>((acc, cur) => {
        ...
        return lastElement?.time === cur.time
            ? [...acc.slice(0, acc.length - 1), newLastElement]
            : [...acc, newLastElement];
    }, []);
};

const validChat = (maybeChat: string) => maybeChat[0] === "[" && maybeChat.includes("<");

export const DATE_FORMAT = "yyyy-MM-dd HH:mm:ss";

const floor = (x: number, unit: number) => Math.floor(x / unit) * unit;
